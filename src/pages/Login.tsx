...
import { Beef, Loader2, Lock, Mail, Github } from "lucide-react";
    ...
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden bg-[#0a0a0a]">
      ...
      <div className="w-full max-w-md px-4 relative z-10 animate-in fade-in zoom-in duration-700">
        <div className="flex flex-col items-center mb-8">
          <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center shadow-lg shadow-emerald-500/20 mb-4 transform hover:scale-110 transition-transform cursor-default">
            <Beef className="h-8 w-8 text-white" />
          </div>
          <h1 className="text-3xl font-heading font-bold tracking-tight text-white">AbateSmart</h1>
          <p className="text-muted-foreground mt-2">Gestão inteligente para frigoríficos</p>
        </div>
        ...
        <Card className="border-border/40 bg-[#121212]/60 backdrop-blur-2xl shadow-2xl">
          ...
          <form onSubmit={handleAuth}>
            ...
            <CardFooter className="flex flex-col space-y-4">
              <Button 
                type="submit" 
                className="w-full bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-600/20 py-6 text-lg font-semibold transition-all hover:scale-[1.02] active:scale-[0.98]"
                disabled={loading}
              >
                {loading ? (
                  <>
              ...
              <div className="grid grid-cols-2 gap-4 w-full">
                <Button variant="outline" className="border-border/40 bg-background/20 hover:bg-background/40" type="button">
      ...
      {/* Footer info */}
      <div className="absolute bottom-4 left-0 w-full text-center text-[10px] text-muted-foreground/30 uppercase tracking-[0.2em]">
        AbateSmart ERP v2.0 • Sistema Seguro • © 2026
      </div>
    </div>
  );
}
